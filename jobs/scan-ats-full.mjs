...
import { loadSeenUrls, appendToPipeline, appendToScanHistory } from './scan.mjs';
import { SEED_SOURCES, toPortalEntry } from './seeds/vc-portfolios.mjs';
import { PORTALS_PATH, PIPELINE_PATH, CACHE_DIR } from './lib/paths.mjs';
const ATS_CACHE_DIR = path.join(CACHE_DIR, 'ats-companies');
...
const SOURCES = {
  ...
  rippling: {
    provider: rippling,
    dataset: null,
    openjobdataAts: 'rippling',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://ats.rippling.com/${slug}/jobs`, h => h === 'ats.rippling.com')
      : null,
  },
  workable: {
    provider: workable,
    dataset: null,
    openjobdataAts: 'workable',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://apply.workable.com/${slug}`, h => h === 'apply.workable.com')
      : null,
  },
  bamboohr: {
    provider: bamboohr,
    dataset: null,
    openjobdataAts: 'bamboohr',
    toEntry: (slug) => {
      const s = String(slug).toLowerCase();
      if (!/^[a-z0-9][a-z0-9-]*$/.test(s)) return null;
      return entryOnHost(s, `https://${s}.bamboohr.com/careers`, h => h === `${s}.bamboohr.com`);
    },
  },
  smartrecruiters: {
    provider: smartrecruiters,
    dataset: null,
    openjobdataAts: 'smartrecruiters',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://careers.smartrecruiters.com/${slug}`, h => h === 'careers.smartrecruiters.com')
      : null,
  },
  recruitee: {
    provider: recruitee,
    dataset: null,
    openjobdataAts: 'recruitee',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://${slug}.recruitee.com`, h => h.endsWith('.recruitee.com'))
      : null,
  },
  pinpoint: {
    provider: pinpoint,
    dataset: null,
    openjobdataAts: 'pinpoint',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://${slug}.pinpointhq.com`, h => h.endsWith('.pinpointhq.com'))
      : null,
  },
  teamtailor: {
    provider: teamtailor,
    dataset: null,
    openjobdataAts: 'teamtailor',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://${slug}.teamtailor.com`, h => h.endsWith('.teamtailor.com'))
      : null,
  },
  personio: {
    provider: personio,
    dataset: null,
    openjobdataAts: 'personio',
    toEntry: (slug) => SLUG_RE.test(String(slug))
      ? entryOnHost(String(slug), `https://${slug}.jobs.personio.de`, h => h.endsWith('.jobs.personio.de') || h.endsWith('.jobs.personio.com'))
      : null,
  },
};
...
async function loadCompanyList(name, source) {
  mkdirSync(ATS_CACHE_DIR, { recursive: true });
  const cacheFile = path.join(ATS_CACHE_DIR, `${name}.json`);
  ...
  if (source.dataset) {
    try {
      const data = await fetchJson(source.dataset, { timeoutMs: 30_000 });
...
async function main() {
  ...
  const logFlags = parseLogFlags(process.argv.slice(2));
  const logger = createLogger('scan-ats', {
    json: opts.json,
    quiet: logFlags.quiet,
    verbose: opts.verbose || logFlags.verbose,
    log: logFlags.log,
  ...
  const log = (...a) => logger.info(a.join(' '));
  const progress = (s) => { if (!opts.json) process.stdout.write(s); };

  if (!existsSync(PORTALS_PATH)) {
    console.error(`Error: portals config not found at ${PORTALS_PATH}. Copy config/portals.example.yml to config/portals.yml`);
    process.exit(1);
  }
  ...
  for (const name of opts.ats) {
    ...
    const { list, status } = await loadCompanyList(name, source);
  ...
  logger.stat('companiesScanned', totalCompaniesScanned);
  logger.stat('unreachableBoards', totalErrors);
  logger.summary();
  logger.close();
